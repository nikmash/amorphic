...
function buildAppConfigAndLoadTemplates(appName, config, controllerJsDir, commonJsDir, sessionStore,
    ...
    return config.appConfig && config.appConfig.templateMode === 'typescript' ? loadTSTemplates(appName) : loadTemplates(appName);
}
...
/**
 * Loads templates for an app at startup time.
 *
 * @param {String} appName - The app name.
 *
...
function loadTSTemplates(appName) {
    ...
    let appConfig = AmorphicContext.applicationConfig[appName] || {};
    let controllerPath = (appConfig.appConfig.controller || 'controller.js');
    if (!controllerPath.match(/(.*?)([0-9A-Za-z_]*\.ts)$/))
        throw "controllerPath (" + controllerPath + ") must specify a .ts file with templateMode of typescript";
    let controllerJS = matches[2];
    let baseTemplate = buildBaseTemplate(appConfig);
    ...
    AmorphicContext.applicationTSController[appName] = require(controllerPath.replace(/.ts$/, '.js'));

    /* TODO: Typescript
       We need to look at the decorator breadcrumbs in prototype.__amorphic__ and follow the dependency chain
       injecting into each template all of the things needed to make it an Amorphic template.
       (see what supertype and semotus do for this).
    */
