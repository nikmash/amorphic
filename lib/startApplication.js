...
function buildAppConfigAndLoadTemplates(appName, config, controllerJsDir, commonJsDir, sessionStore,
    ...
    return config.templateMode === 'typescript' ? loadTSTemplates(appName) : loadTemplates(appName);
}
...
function loadTSTemplates(appName) {
    ...
    let appConfig = AmorphicContext.applicationConfig[appName] || {};
    let controllerPath = (appConfig.appConfig.controller || 'controller.ts');
    if (!controllerPath.match(/(.*?)([0-9A-Za-z_]*\.ts)$/))
        throw new Error("controllerPath (" + controllerPath + ") must specify a .ts file with templateMode of typescript");
    let baseTemplate = buildBaseTemplate(appConfig);
    ...
    AmorphicContext.applicationTSController[appName] = require(appConfig.appPath + '/' + controllerPath.replace(/.ts$/, '.js')).Controller;
    baseTemplate.getClasses();
...
function buildBaseTemplate(appConfig) {
    ...
    var baseTemplate = persistor(null, null, semotus);
